...
{
public:
    explicit CustomLayerPlugin(float alpha) noexcept;
    CustomLayerPlugin(CustomLayerPlugin const& other) noexcept;
    ...
    int32_t getOutputDataTypes(nvinfer1::DataType* outputTypes,
        int32_t nbOutputs, nvinfer1::DataType const* inputTypes,
        int32_t nbInputs) const noexcept override;
    int32_t getOutputShapes(nvinfer1::DimsExprs const* inputs,
        int32_t nbInputs, nvinfer1::DimsExprs const* shapeInputs,
    ...
    int32_t onShapeChange(nvinfer1::PluginTensorDesc const* inputs,
        int32_t nbInputs, nvinfer1::PluginTensorDesc const* outputs,
        int32_t nbOutputs) noexcept override;
...
private:
    void resetSerializationFields() noexcept;

    float alpha_{};
    nvinfer1::PluginField serializedAlpha_{};
    nvinfer1::PluginFieldCollection serializedFields_{};
};

class CustomLayerPluginCreator final : public nvinfer1::IPluginCreatorV3One
{
public:
...
private:
    std::vector<nvinfer1::PluginField> attributes_;
    nvinfer1::PluginFieldCollection fieldNames_{};
};

// Calling this function forces the static plugin registrar in the library to
// be linked into each executable.
